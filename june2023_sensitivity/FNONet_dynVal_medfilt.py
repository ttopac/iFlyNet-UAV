...
# %%
training_dir = "/home/tanay/onedrive_su/MLmodels/Training1_Jan2023"
dyn_dir = "/home/tanay/onedrive_su/MLmodels/Dynamic1_Jan2023"
...
# %%
# Apply filtering to the PZT data
for i in range(7):
  allExamples[:,i:i+1,:] = signal.medfilt(allExamples[:,i:i+1,:], (1,1,9))
  print (f"Median filtering completed for PZT{i+1}")
...
# %%
# OPTION 1:
# Pre-process dynamic data labels (work with INTEGER data)
# with open(os.path.join(dyn_labels_dir,'groundtruth_run15.pkl'), 'rb') as f:
#     dyn_labels = pickle.load(f)
# int_airspeed_labels = dyn_labels["WT_Airspeed_int"].apply(lambda x: str(int(x))).to_numpy(dtype=object)
# int_aoa_labels = dyn_labels["WT_AoA_int"].apply(lambda x: str(int(x))).to_numpy(dtype=object)
# dynState = int_airspeed_labels+"m/s_"+int_aoa_labels+"deg"
# dynState = dynState.astype('|S20')

# #Filter the dynamic data so we only take the samples with seen labels
# contained_indices = np.where(np.in1d(dynState, trainingState))[0]
# mask = np.zeros(len(dynState), bool)
# mask[contained_indices] = 1
# contained_dyn_state = dynState[mask][0:len(dyn_data_transposed)]
# contained_dyn_data_transposed = dyn_data_transposed[mask[0:len(dyn_data_transposed)], :, :]

# OPTION 2:
# Pre-process dynamic data labels (work with FLOAT data)
with open(os.path.join(dyn_labels_dir,'dynamic1_run15_truth.pkl'), 'rb') as f:
    dyn_labels = pickle.load(f)
dyn_labels = dyn_labels[0:splitparameter]    
float_airspeed_labels = dyn_labels["WT_Airspeed"].apply(lambda x: str(float(x))).to_numpy(dtype=object)
float_aoa_labels = dyn_labels["WT_AoA"].apply(lambda x: str(float(x))).to_numpy(dtype=object)
dynState = float_airspeed_labels+"m/s_"+float_aoa_labels+"deg"

#Filter the dynamic data so we only take the samples with seen labels
airspeed_lowerbound = float_airspeed_labels.astype('float')>=7
airspeed_upperbound = float_airspeed_labels.astype('float')<=20
aoa_lowerbound = float_aoa_labels.astype('float')>=0
aoa_upperbound = float_aoa_labels.astype('float')<=16
...
contained_dyn_state = dynState[contain_ixs]
contained_dyn_data_transposed = dyn_data_transposed[contain_ixs[0:len(dyn_data_transposed)], :, :]

# %%
#Convert string-based state info to 2 integer values (Airspeed and AoA)
def convert_state(state_str):
  state_str = state_str.astype(str)
  splitted = np.char.split(state_str, sep="m")
  splitted_2d = np.stack(splitted, axis=0)
  vel = splitted_2d[:,0].astype(float)

  splitted2 = np.char.split(state_str, sep="_")
  splitted2_2d = np.stack(splitted2, axis=0)
  secondhalf = np.char.split(splitted2_2d[:,1], sep="d")
  secondhalf_2d = np.stack(secondhalf, axis=0)
  aoa = secondhalf_2d[:,0].astype(float)
  return vel, aoa

training_vels, training_aoas = convert_state(trainingState)
uncontained_dyn_vels, uncontained_dyn_aoas = convert_state(dynState)
contained_dyn_vels, contained_dyn_aoas = convert_state(contained_dyn_state)

# %%
# Finalize the training data
trainingX = examples
trainingY = np.stack((training_vels, training_aoas), axis=1)
...
for epoch in range(0, num_epochs):  # loop over the dataset
  ...
  with torch.no_grad():
    val_output = fnonet_1d.directstep(net.cuda(), val_data)
...
unfiltered_dyn_labels = np.stack((uncontained_dyn_vels, uncontained_dyn_aoas), axis=1)
...
filtered_dyn_labels = np.stack((contained_dyn_vels, contained_dyn_aoas), axis=1)
