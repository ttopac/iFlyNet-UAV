...
MODELS_LOC = os.path.join(AUG2022_LOC, "KerasML", "Training1_Jan2023", "models")
DYNAMICTEST_LOC = os.path.join(AUG2022_LOC, "KerasML", "Dynamic1_Jan2023")
...
INCLUDE_STALL_PRED = False
INCLUDE_LIFTDRAG_PRED = False
LIFTDRAG_PRED_METH = 'vlm' #choose among vlm and others (TODO)

MODEL_NAME = "model_ResNet18_dynVal_ph10"
DYNAMICTEST_NAME = "dynamic1_run15"
ENCODER_NAME = "encoder_ResNet18_dynVal"
...
with open(os.path.join(DYNAMICTEST_LOC, "labels", DYNAMICTEST_NAME+"_truth.pkl"), 'rb') as f:
...
airspeed_truth, aoa_truth = offline_other_sources.get_truth_state()

# %%
# Bring in the i-FlyNet predictions
dynamictest_preds_loc = os.path.join(DYNAMICTEST_LOC, "predictions")
offline_preds = ProcPredsOffline(test_data, dynamictest_preds_loc, DYNAMICTEST_NAME)

# State predictions:
if INCLUDE_STATE_PRED:
  airspeed_preds, aoa_preds = offline_preds.get_preds_state(MODELS_LOC, MODEL_NAME, ENCODER_NAME)
...
airspeed_uav, aoa_uav = offline_other_sources.get_uav_state()
...
stream = StreamOffline(streamhold_queue)

if INCLUDE_STATE_PRED:
  stream.prep_data_state(airspeed_truth, aoa_truth, airspeed_preds, aoa_preds, airspeed_uav, aoa_uav)
...
GUIapp.draw_midrow("Flight Characteristics", os.path.join(GUI_LOC, "assets", "legend.png"))

stream.initialize_video(GUIapp, "Experiment Camera", VIDEO_LOC, DYNAMICTEST_NAME)
stream.initialize_plots_wcomparison(GUIapp, INCLUDE_STATE_PRED, INCLUDE_LIFTDRAG_PRED, PLOT_REFRESH_RATE, VISIBLE_DURATION, DATA_PER_SECOND)
